...
void minHeapify(struct MinHeap* minHeap, int idx)
{
    ...
    if (smallest != idx)
    {
        ...
        swap(&minHeap->array[smallest], &minHeap->array[idx]); minHeapify(minHeap, smallest);
    }
}
...
void writeBit(ofstream& file2, int bit)
{
        ...
        if (bit)
            bit_buffer |= (1 << current_bit); current_bit++;
        if (current_bit == 8)
            {
    ...
    while (getline(file1, line))
    {
        ...
        for (char ch : line)
        {
            ...
            for (int i = 0; i < bin_code.length(); i++)
            {
                ...
                writeBit(file2, bit); out += bin_code[i];
            }
        }
    }
...
string convert_to_string(string name, int should_get_header, int& padding,map<char, string>& codes)
{
...
if (should_get_header)
{
    ...
    for (int i = 0; i < size; i++)
    {
        ...
        char* value = (char*)malloc(sz + 1); fin.read(value, sz);
        value[sz] = '\0';
        codes[a] = value;
...
void decodeRealWorld(string in, ofstream& fout, map<char, string> codes,int padding)
{
